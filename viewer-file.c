...
static ViewerFile *viewer_file_real_close(ViewerFile *const,const GError *const *const);
...
// Signals
  ...
  static guint file_signals[N_SIGNALS]={};
...
// Construction
  ...
  static void viewer_file_class_init(ViewerFileClass *const klass){
    ...
    // Signals
    file_signals[SIG_CHANGED]=g_signal_newv(
      "changed",
...
// Method
  ...
  ViewerFile *viewer_file_write(ViewerFile *const self,const guint8 *const buffer,const gsize size){
    g_assert( VIEWER_IS_FILE (self) && buffer && size>=1 );
    /* First write data. */
    g_print("Viewer::File::write()\n");
    /* Then, notify user of data written. */
    g_signal_emit(self,file_signals[SIG_CHANGED],0/* details */);
    return self;
  }
...
// Interface Viewer::File::Editable
...
// Interface Viewer::File::EditableLossy
